...
function NTserveOp({submithandler})
{
  const [yvalue, sety] = useState(true);
  let obj
  if(!yvalue)
  obj={tab:'staff_service',x:"departmentId",y:'Averge Years Of Experience'} 
  else
  obj={tab:'staff_service',x:"gender",y:"No of Staff",dep:"all"}
  ...
  function handle(event){
    event.persist()
        if(event.target.value==='departmentId'){
          ...
          sety(true);
          obj={tab:'staff_service',x:event.target.value,y:"No of Staff",dep:'all'}
          setIt(obj)
        }
  }
                ...
                yvalue ?
                    ...
                    (
                        <div className="together">
                            <h5>y:</h5>
                            <select className="select-css" id="y" name="y" onChange={handleChange} >
                                <option value='Averge Years Of Experience'>avg number of years of experience</option>
                            </select>
                        </div>
                    )
            }
